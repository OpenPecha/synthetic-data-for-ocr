...
def apply_random_augmentation(image: np.array) -> tuple:
    augmentations = [
        (A.RandomBrightnessContrast(p=1.0), "RandomBrightnessContrast"),
        (A.HueSaturationValue(p=1.0), "HueSaturationValue"),
        (A.RGBShift(p=1.0), "RGBShift"),
        (A.RandomGamma(p=1.0), "RandomGamma"),
        (A.GaussianBlur(p=1.0), "GaussianBlur")
    ]
    augmentation, aug_name = random.choice(augmentations)
    augmented = augmentation(image=image)
    return augmented['image'], aug_name
...
class SyntheticPageImageGenerator:
    ...
    def save_page_image(self, image: np.array, base_filename: str, augmentation_name: str):
        filename = f"{base_filename}_{augmentation_name}.png"
        cv2.imwrite(os.path.join(self.page_dir, filename), image)
        return filename

    def extract_lines(self, page_image: np.array, text: str, font: ImageFont, page_filename: str):
        lines = text.split("\n")
        y = 10
        for line_idx, line in enumerate(lines):
            ...
            if line_image.size == 0:  # Check if the cropped image is empty
                continue
            line_filename = f"{os.path.splitext(page_filename)[0]}_line_{line_idx}.png"
            cv2.imwrite(os.path.join(self.line_dir, line_filename), line_image)

    def run(self, text_folder: str):
        ...
        for text_file in tqdm(text_files):
            text = "\n\n".join(read_file(text_file))
            base_filename = get_filename(text_file)
            for font in fonts:
                background = None
                if self.backgrounds:
                    background = cv2.imread(random.choice(self.backgrounds))
                page_image = self.generate_page_image(text, font, background)
                augmented_page_image, aug_name = apply_random_augmentation(page_image)
                page_filename = self.save_page_image(augmented_page_image, base_filename, aug_name)
                self.extract_lines(augmented_page_image, text, font, page_filename)
...
output_dir = "/Users/ogyenthoga/Desktop/Work/synthetic-data-for-ocr/output"
